...
byte Name1[4]={0xB9,0x2C,0xD0,0xB8};//first UID card
byte Name2[4]={0xC3,0x7A,0xC9,0x73};//second UID card  //C3 7A C9 73
byte Name3[4]={0x44,0x69,0x6D,0x25};// this is new one
byte Name4[4]={0x76,0x9F,0xA6,0x23};// this is new one 
...
void setup() {
  ...
  Serial.println("LABEL,Date,Name,Number");// make four columns (Date,Time,[Name:"user name"]line 48 & 52,[Number:"user number"]line 49 & 53)
...
void loop() {
 ...
 for (byte i = 0; i < mfrc522.uid.size; i++) {
     card_ID[i]=mfrc522.uid.uidByte[i];
       ...
       Name="Tejas Sir";//user name
       Number=45;//user number
       j=0;//first number in the NumbCard array : NumbCard[j]
      }
      else if(card_ID[i]==Name2[i]){
       Name="Om Sir";//user name
       Number=05;//user number
